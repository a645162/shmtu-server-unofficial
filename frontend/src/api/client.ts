...
export const authApi = {
  ...
  me: () => apiClient.get<User>('/auth/me').then(extractData),
  changePassword: (data: ChangePasswordRequest) =>
    apiClient.put('/auth/password', data).then(extractData),
};

export const accountApi = {
  list: () => apiClient.get<Account[]>('/accounts').then(extractData),
  get: (id: number) => apiClient.get<Account>(`/accounts/${id}`).then(extractData),
  ...
  delete: (id: number) => apiClient.delete(`/accounts/${id}`).then(extractData),
  login: (id: number) => apiClient.post(`/accounts/${id}/login`).then(extractData),
  sync: (id: number) => apiClient.post(`/accounts/${id}/sync`).then(extractData),
...
export const billApi = {
  ...
  newBills: () => apiClient.get<BillOriginal[]>('/bills/new').then(extractData),
  markRead: (billIds?: number[]) =>
    apiClient.post('/bills/mark-read', billIds ? { billIds } : undefined).then(extractData),
};

export const notificationApi = {
  list: () => apiClient.get<NotificationLog[]>('/notifications').then(extractData),
  channels: () => apiClient.get<ChannelInfo[]>('/notifications/channels').then(extractData),
  test: () => apiClient.post('/notifications/test').then(extractData),
  getSettings: () =>
...
export const configApi = {
  list: () => apiClient.get<SystemConfig[]>('/config').then(extractData),
  update: (key: string, data: UpdateConfigRequest) =>
    apiClient.put(`/config/${key}`, data).then(extractData),
  getOcr: () => apiClient.get('/config/ocr').then(extractData),
  updateOcr: (data: Record<string, unknown>) =>
    apiClient.put('/config/ocr', data).then(extractData),
  getScheduler: () => apiClient.get('/config/scheduler').then(extractData),
  updateScheduler: (data: Record<string, unknown>) =>
    apiClient.put('/config/scheduler', data).then(extractData),
};

export const systemApi = {
  health: () => apiClient.get<SystemHealth>('/system/health').then(extractData),
  stats: () => apiClient.get<SystemStats>('/system/stats').then(extractData),
  ...
  syncAll: () => apiClient.post('/system/sync-all').then(extractData),
};

export const dashboardApi = {
  stats: () => apiClient.get<DashboardStats>('/dashboard/stats').then(extractData),
  recentBills: () =>
    apiClient.get<PageResponse<BillOriginal>>('/bills', { params: { size: 5 } })
      .then((r) => {
        const d = extractData(r) as unknown;
        if (d && typeof d === 'object' && 'content' in (d as object)) {
          return (d as PageResponse<BillOriginal>).content;
        }
        return d as BillOriginal[];
      }),
};
