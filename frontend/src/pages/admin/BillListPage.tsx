...
const BillListPage: React.FC = () => {
  ...
  const fetchBills = async (pageNum: number = 0) => {
    setLoading(true);
    try {
      const params: BillQueryParams = { page: pageNum, size: 20 };
      ...
      <div className={styles.filters}>
        <div className={styles.filterItem}>
          ...
          <Input
            ...
            onChange={(_, d) => setFilterCategory(d.value)}
            placeholder="如: 食堂"
          />
        </div>
        <div className={styles.filterItem}>
          ...
          <Input
            ...
            onChange={(_, d) => setFilterStartDate(d.value)}
          />
        </div>
        <div className={styles.filterItem}>
          <Label>结束日期</Label>
          <Input
            type="date"
            value={filterEndDate}
            onChange={(_, d) => setFilterEndDate(d.value)}
          />
        </div>
        <div className={styles.filterItem}>
          <Label>状态</Label>
          <Select value={filterIsNew} onChange={(_, d) => setFilterIsNew(d.value)}>
